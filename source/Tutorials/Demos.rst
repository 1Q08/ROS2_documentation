演示
====

.. toctree::
   :maxdepth: 1

   Demos/Quality-of-Service
   Demos/Managed-Nodes
   Demos/Intra-Process-Communication
   Demos/Rosbag-with-ROS1-Bridge
   Demos/Real-Time-Programming
   Demos/dummy-robot-demo
   Demos/Logging-and-logger-configuration
   Demos/Content-Filtering-Subscription
   Demos/Wait-for-Acknowledgment

外部资源
--------

* `ROS 1 与 ROS 2 之间的通信桥接 <https://github.com/ros2/ros1_bridge/blob/master/README.md>`__
* `MoveIt 2 机械臂的运动规划 <https://moveit.picknik.ai/main/index.html>`__
* 使用 Turtlebot 3（社区贡献）

    - `入门 <https://emanual.robotis.com/docs/en/platform/turtlebot3/quick-start/>`__
    - `仿真 <https://ubuntu.com/blog/simulate-the-turtlebot3>`__
    - `在仿真中导航 <https://github.com/cyberbotics/webots_ros2/wiki/Navigate-TurtleBot3>`__
    - `在仿真中学习 SLAM <https://github.com/cyberbotics/webots_ros2/wiki/SLAM-with-TurtleBot3>`__

